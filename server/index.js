...
app.post('/encrypt', (req, res) => {
    uploadFile(req, res, (error) => {
        ...
        const filePath = path.join(`${__dirname}/storage/${req.file.filename}`);
        const op = "encrypt";
...
app.post('/decrypt', (req, res) => {
  uploadFile(req, res, (error) => {
      ...
      const filePath = path.join(`${__dirname}/storage/${req.body.file}`);
      const filename = path.parse(req.body.file).name;
      const outputFilePath = path.join(`${__dirname}/storage/decrypted/${filename}`);
...
app.get('/files', (req, res) => {
  const files = fs.readdirSync(__dirname + "/storage").filter(e => {
